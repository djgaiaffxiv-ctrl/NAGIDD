...
let BIN, OUT, YTDLP, FFDIR, ARIA, DENO;
function init(cfg) {
  cfg = cfg || {};
...
async function startDownload(url, fmt, album, cookies, log) {
  ...
  if (fmt === 'mp3') {
    ...
    log('-> Instagram (fotos, videos, reels, stories)...');
  ...
  // Cookies: archivo, Firefox (lectura directa), o Chromium (extraccion automatica via CDP)
  let cookieArgs = null;
  if (cookies && cookies.enabled) {
    if (cookies.file) {
      log('Usando archivo de cookies: ' + path.basename(cookies.file));
      cookieArgs = ['--cookies', cookies.file];
    } else {
      const br = String(cookies.browser || 'brave').toLowerCase();
      if (br === 'firefox') {
        log('Usando cookies de Firefox...');
        cookieArgs = ['--cookies-from-browser', 'firefox'];
      } else if (cookiesLib.isChromium(br)) {
        const f = await cookiesLib.getCookiesFile(br, log);
        if (f) cookieArgs = ['--cookies', f];
        else log('Sigo sin cookies (no servira para videos privados o +18).');
      } else {
        cookieArgs = ['--cookies-from-browser', br];
      }
    }
  }
  let r = cookieArgs ? await runYtDlp(cookieArgs.concat(a), log) : await runYtDlp(a, log);

  // Rescate KVS (no aplica a Instagram/X)
  if (r.exit !== 0 && !/cookie/i.test(r.err) && !canceled && fmt !== 'instagram') {
...
async function runBatch(payload, log) {
  ...
  for (const u of urls) {
    ...
    if (r.exit === 0) ok++;
      ...
      const el = (r.err.split(/\r?\n/).filter((l) => /ERROR/.test(l)).pop() || '(codigo ' + r.exit + ')').trim();
