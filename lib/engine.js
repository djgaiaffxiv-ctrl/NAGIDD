...
// Recorte por tiempo: convierte inicio/fin en la seccion de yt-dlp "*inicio-fin".
// Acepta SS, M:SS, MM:SS, H:MM:SS (y coma como separador). Vacio = desde 0 / hasta el final.
function normTime(t) {
  t = String(t == null ? '' : t).trim().replace(/,/g, ':').replace(/\./g, ':');
  if (!t) return '';
  if (!/^\d{1,3}(:\d{1,2}){0,2}$/.test(t)) return null;  // formato invalido
  return t;
}
function trimSection(trim) {
  if (!trim) return null;
  const s = normTime(trim.start), e = normTime(trim.end);
  if (s === null || e === null) return null;     // algo invalido -> sin recorte
  if (!s && !e) return null;                      // ambos vacios -> video entero
  return '*' + (s || '0') + '-' + (e || 'inf');
}

// Descifrado KVS (Kernel Video Sharing) - port exacto de yt-dlp generic.py (_kvs_get_real_url).
// Muchos tubes (sexvid.xxx, etc.) ofuscan el mp4 como 'function/0/<url>' + un license_code.
function kvsLicenseToken(licenseCode) {
  licenseCode = String(licenseCode).replace(/\$/g, '');
  const values = licenseCode.split('').map((c) => parseInt(c, 10));
  ...
  for (let index = 0; index < mod.length; index++) {
    ...
    for (let offset = 0; offset < 4; offset++) token.push((values[index + offset] + current) % 10);
  }
  return token;
}
function kvsRealUrl(videoUrl, licenseCode) {
  if (!videoUrl || !videoUrl.startsWith('function/0/')) return videoUrl;  // no ofuscado
  let u;
  try { u = new URL(videoUrl.slice('function/0/'.length)); } catch (_) { return null; }
  const token = kvsLicenseToken(licenseCode);
  const parts = u.pathname.split('/');           // ['', 'get_file', '9', '<hash>', ...]
  const HL = 32;
  const hash = parts[3].slice(0, HL);
  const indices = Array.from({ length: HL }, (_, i) => i);
  let accum = 0;
  for (let src = HL - 1; src >= 0; src--) {
    accum += token[src];
    const dest = (src + accum) % HL;
    const t = indices[src]; indices[src] = indices[dest]; indices[dest] = t;
  }
  let nh = '';
  for (let i = 0; i < HL; i++) nh += hash[indices[i]];
  parts[3] = nh + parts[3].slice(HL);
...
async function startDownload(url, fmt, album, cookies, log, trim) {
  ...
  // Normalizacion
  const before = url;
  // goldtits.com (y agregadores de su red) son paginas-puente que redirigen al tube real
  // (pornhat, txxx, sexvid...). yt-dlp falla con la URL puente -> seguimos el redirect primero.
  if (/goldtits\.com/i.test(url)) {
    try {
      const resp = await fetch(url, { redirect: 'follow', headers: { 'User-Agent': UA } });
      if (resp && resp.url && resp.url !== url) url = resp.url;
    } catch (_) {}
  }
  url = url.replace(/:\/\/(www\.)?txxx\.me\//i, '://txxx.com/');   // mirror no soportado -> dominio que si va
  url = url.replace(/[?&](promo|ad_sub)=[^&#]*/ig, '').replace(/\?&/, '?').replace(/[?&]$/, '');
  ...
  // Organizacion por tipo: MP3 (sueltas + playlists) -> Musica ; el resto (video) -> videos
  const typeRoot = (fmt === 'mp3') ? 'Música' : 'videos';
  const base = path.join(OUT, typeRoot);
  // subcarpeta: album fijo, o titulo de la playlist (opcional); las sueltas van directas en el tipo
  const subTok = safeAlbum ? (safeAlbum + '\\') : '%(playlist_title|)s\\';
  const folderName = safeAlbum ? path.join(typeRoot, safeAlbum) : typeRoot;
  log('Carpeta: ' + folderName);
  const tpl = base + '\\' + subTok + '%(playlist_index&{:02d} - |)s%(title)s.%(ext)s';
  ...
  // Recorte por tiempo (inicio/fin): yt-dlp baja solo esa seccion. Las secciones van por
  // ffmpeg, asi que NO usamos aria2c (se quedaria sin el rango).
  const section = trimSection(trim);
  if (section) {
    log('Recorte: ' + section.slice(1).replace('-inf', ' -> final').replace('-', ' -> '));
    a = ['--download-sections', section, '--extractor-args', 'generic:impersonate'].concat(a);
  } else {
  ...
  if (r.exit !== 0 && !/cookie/i.test(r.err) && !canceled && fmt !== 'instagram' && fmt !== 'facebook' && fmt !== 'x') {
    ...
    if (html) {
      const lic = (html.match(/license_code\s*:\s*["']([^"']+)["']/) || [])[1] || '';
      const cands = [];
      const re = /video(?:_alt)?_url\d*\s*:\s*["']([^"']+)["']/g;
      let mm;
      while ((mm = re.exec(html))) {
        let v = mm[1];
        if (v.startsWith('function/0/')) v = kvsRealUrl(v, lic);  // KVS cifrado -> descifrar con license_code
        if (v && /^https?:\/\//.test(v)) cands.push(v);
      }
      if (cands.length) {
        murl = cands.sort((x, y) => {
...
async function runBatch(payload, log) {
  ...
  for (const u of urls) {
    ...
    let r = await startDownload(u, fmt, album, cookies, log, trim);
    ...
    if (r.exit !== 0 && /403|429|503|522|Cloudflare|Too Many Requests|rate.?limit/i.test(r.err) && !canceled) {
      ...
      r = await startDownload(u, fmt, album, cookies, log, trim);
    }
    if (r.exit === 0) ok++;
    else {
