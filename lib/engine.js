...
async function startDownload(url, fmt, album, cookies, log) {
  ...
  if (cookies && cookies.enabled) {
    ...
    log('Usando cookies del navegador: ' + br + '  (cierra ' + br + ' si da error de cookies)');
    r = await runYtDlp(['--cookies-from-browser', br].concat(a), log);
    if (r.exit !== 0 && /cookie/i.test(r.err)) {
      log('No pude leer las cookies de ' + br + '. Reintento SIN cookies...');
      r = await runYtDlp(a, log);
    }
  } else {
    r = await runYtDlp(a, log);
  }
